...
([function (e, t) {
            ...
            AFRAME.registerComponent("environment", {
                ...
                init: function () {
                    ...
                    for (var e in this.assets)
                        for (var t = 0; t < this.assets[e].length; t++) {
                            ...
                            if ("mesh" == r.type)
                                for (var o = 0, i = r.vertices.length; o < i; o++)
                                    r.vertices[o] /= 1e3
                        }
                ...
                getFogColor: function (e, t) {
                    var r;
                    if ("color" == e || "none" == e)
                        r = (new THREE.Color).setStyle(this.environmentData.skyColor, THREE.NoColorSpace);
                    else if ("gradient" == e)
                        r = (new THREE.Color).setStyle(this.environmentData.horizonColor, THREE.NoColorSpace);
                    ...
                    r.lerp((new THREE.Color).setStyle(this.data.groundColor, THREE.NoColorSpace), .3),
                    r.setRGB(r.r, r.g, r.b, THREE.SRGBColorSpace),
                    "#" + r.getHexString()
                },
                update: function (e) {
                    var t;
                    ...
                    if (o.normalize(), this.sunlight) {
                        if (this.sunlight.setAttribute("position", this.environmentData.lightPosition), "atmosphere" != r) {
                            ...
                            i.setRGB((i.r + 1) / 2, (i.g + 1) / 2, (i.b + 1) / 2, THREE.SRGBColorSpace),
                            this.hemilight.setAttribute("light", {
                    ...
                    if (n && (this.STAGE_SIZE = this.data.stageSize, this.sky.setAttribute("radius", this.STAGE_SIZE)), r !== t.skyType || this.environmentData.skyColor != t.skyColor || this.environmentData.horizonColor != t.horizonColor) {
                        ...
                        "color" == r ? (s.color = this.environmentData.skyColor, s.fog = !1) : "gradient" == r && (s.topColor = (new THREE.Color).setStyle(this.environmentData.skyColor, THREE.NoColorSpace), s.bottomColor = (new THREE.Color).setStyle(this.environmentData.horizonColor, THREE.NoColorSpace)),
                        this.sky.setAttribute("material", s)
                    }
                    "atmosphere" == r && (this.sky.setAttribute("material", {
                            sunPosition: o
                        }), this.setStars(2e3 * (1 - Math.max(0, 8 * (o.y + .08))))),
                ...
                remove: function () {
                    ...
                    this.groundTexture && this.groundTexture.dispose(),
                    this.gridTexture && this.gridTexture.dispose(),
                    this.groundMaterial && this.groundMaterial.dispose(),
                    this.groundGeometry && this.groundGeometry.dispose(),
                    this.el.removeChild(this.ground);
                    var e = this.dressing.getObject3D("mesh");
                    e && e.children.length > 0 && (e.children[0].material.dispose(), e.children[0].geometry.dispose()),
                    ...
                    for (var o in this.schema)
                        if (!("preset" == o || r && void 0 === r[o])) {
                            ...
                            if ("vec3" == s) {
                                ...
                                i != n && ("number" == this.schema[o].type && (n = e(n)), t.push(o + ": " + n))
                        }
                ...
                updateGround: function (e) {
                    ...
                    if (e) {
                        ...
                        if (this.ground.setAttribute("visible", o), !o)
                            return;
                        ...
                        for (var i = new r(this.environmentData.seed), n = this.groundGeometry.attributes.position.array, s = n.length, a = this.environmentData.groundFrequency, l = a / t, d = 0, h = 0, g = 2; g < s; g += 3)
                            if ("flat" != this.environmentData.ground) {
                                ...
                                c = Math.max(0, Math.min(1, (Math.abs(c) - (f - .9)) * (1 / f))),
                ...
                drawGrid: function (e, t, r) {
                    if ("none" != this.environmentData.grid) {
                        ...
                        case "1x1":
                        case "2x2":
                            for ("1x1" == this.environmentData.grid && (s *= 2, a = t / r), o = 0; o < s; o++)
                                n = Math.floor(o * a), e.fillRect(0, n, t, 1), e.fillRect(n, 0, 1, t);
                            break;
                        case "crosses":
                        ...
                        case "dots":
                            for (o = 0; o < s + 1; o++)
                                for (i = 0; i < s + 1; i++)
                                    e.beginPath(), e.arc(Math.floor(i * a), Math.floor(o * a), 4, 0, 2 * Math.PI), e.fill();
                            break;
                        case "xlines":
                            for (o = 0; o < s; o++)
                                e.fillRect(Math.floor(o * a), 0, 1, t);
                            break;
                        case "ylines":
                            for (o = 0; o < s; o++)
                                e.fillRect(0, Math.floor(o * a), t, 1)
                        }
                    }
                },
                drawTexture: function (e, t, o) {
                    ...
                    if ("none" != this.environmentData.groundTexture)
                        ...
                        case "checkerboard":
                            ...
                            for (i = 0; i < g + 1; i += 2)
                                for (var c = 0; c < g + 1; c++)
                                    e.fillRect(Math.floor((i + c % 2) * u), Math.floor(c * u), Math.floor(u), Math.floor(u));
                            break;
                        case "squares":
                            var m = 16,
                            f = t / m;
                            for (s = (new THREE.Color).setStyle(this.environmentData.groundColor, THREE.NoColorSpace), a = (new THREE.Color).setStyle(this.environmentData.groundColor2, THREE.NoColorSpace), i = 0; i < m * m; i++)
                                n = this.random(i + 3) > .5 ? s.clone() : a.clone(), n.addScalar(.1 * this.random(i + 3) - .05), e.fillStyle = "#" + n.getHexString(THREE.NoColorSpace), e.fillRect(i % m * f, Math.floor(i / m) * f, f, f);
                            break;
                        case "noise":
                            d = e.getImageData(0, 0, t, t),
                            l = d.data,
                            s = (new THREE.Color).setStyle(this.environmentData.groundColor, THREE.NoColorSpace),
                            a = (new THREE.Color).setStyle(this.environmentData.groundColor2, THREE.NoColorSpace);
                            var p = new THREE.Color(a.r - s.r, a.g - s.g, a.b - s.b),
                            y = new r;
                            for (i = 0, c = 0, h = l.length; i < h; i += 4, c++) {
                                var v = y.noise(c % t / t * 3, c / t / t * 3, 0);
                                l[i + 0] = Math.floor(255 * (s.r + p.r * v)),
                                l[i + 1] = Math.floor(255 * (s.g + p.g * v)),
                                l[i + 2] = Math.floor(255 * (s.b + p.b * v))
                            }
                        ...
                        case "walkernoise":
                            ...
                            x.fillStyle = this.environmentData.groundColor,
                            x.fillRect(0, 0, C, C),
                            d = x.getImageData(0, 0, C, C),
                            l = d.data,
                            s = (new THREE.Color).setStyle(this.environmentData.groundColor, THREE.NoColorSpace),
                            a = (new THREE.Color).setStyle(this.environmentData.groundColor2, THREE.NoColorSpace);
                            var S = [],
                            T = 1e3;
                            for (i = 0; i < T; i++)
                                n = s.clone().lerp(a, Math.random()), S.push({
                                    x: Math.random() * C,
                                    y: Math.random() * C,
                            ...
                            e.drawImage(E, 0, 0, t, t)
                        }
                },
                getAssetGeometry: function (e) {
                    function t(e, t) {
                        for (var r = e.attributes.position.array, o = r.length, i = 0; i < o; i += 3)
                            r[i] = (s.random(i) - .5) * t, r[i + 1] = (s.random(i + o) - .5) * t, r[i + 2] = (s.random(i + 2 * o) - .5) * t;
                    ...
                    for (var r, o, i, n = [], s = this, a = 0; a < e.length; a++)
                        if ("lathe" == e[a].type) {
                            var l = -99999,
                            d = [];
                            for (i = e[a].vertices, r = 0; r < i.length; r += 2)
                                d.push(new THREE.Vector2(i[r], i[r + 1])), i[r + 1] > l && (l = i[r + 1]);
                            o = new THREE.LatheGeometry(d, e[a].segments || 8),
                            o.applyMatrix4((new THREE.Matrix4).makeRotationFromEuler(new THREE.Euler(-Math.PI, 0, 0))),
                            o.applyMatrix4((new THREE.Matrix4).makeTranslation(0, l, 0)),
                            o = o.toNonIndexed(),
                            n.push(o)
                        } else if ("extrude" == e[a].type) {
                            var h = new THREE.Shape;
                            for (i = e[a].vertices, r = 0; r < i.length; r += 2)
                                0 == r ? h.moveTo(i[r], i[r + 1]) : h.lineTo(i[r], i[r + 1]);
                            o = new THREE.ExtrudeGeometry(h, {
                                amount: 1,
                                bevelEnabled: !1
                            }),
                            o.applyMatrix4((new THREE.Matrix4).makeRotationFromEuler(new THREE.Euler(-Math.PI / 2, 0, 0))),
                            e[a].noise && t(o, e[a].noise),
                            n.push(o)
                ...
                updateDressing: function () {
                    ...
                    if (this.dressing.setAttribute("visible", "none" != this.environmentData.dressing), "none" != this.environmentData.dressing) {
                        ...
                        case "cubes":
                            ...
                            r[0].applyMatrix4((new THREE.Matrix4).makeTranslation(0, .5, 0));
                            break;
                        case "pyramids":
                        ...
                        case "cylinders":
                            ...
                            break;
                        default:
                            if (r = this.getAssetGeometry(this.assets[this.environmentData.dressing]), !r)
                                return
                        }
                        for (var o = 0, i = 88343; o < this.environmentData.dressingAmount; o++, i++) {
                            var n,
                            s = r[Math.floor(this.random(33 + o) * r.length)].clone(),
                            ...
                            g.compose(new THREE.Vector3(Math.cos(h) * n, 0, Math.sin(h) * n), (new THREE.Quaternion).setFromAxisAngle(new THREE.Vector3(0, 1, 0), (this.random(i + 5) - .5) * l.length() * Math.PI * 2), new THREE.Vector3(a + (c ? u : this.random(i + 6)) * l.x, a + (c ? u : this.random(i + 7)) * l.y, a + (c ? u : this.random(i + 8)) * l.z)),
                            s.applyMatrix4(g),
                            t.push(s)
                        }
                        var m = THREE.BufferGeometryUtils.mergeGeometries || THREE.BufferGeometryUtils.mergeBufferGeometries,
                ...
                createStars: function () {
                    for (var e = 2e3, t = new THREE.BufferGeometry, r = new Float32Array(3 * e), o = this.STAGE_SIZE - 1, i = new THREE.Vector3, n = 0; n < r.length; n += 3)
                        i.set(this.random(n + 23) - .5, this.random(n + 24), this.random(n + 25) - .5), i.normalize(), i.multiplyScalar(o), r[n] = i.x, r[n + 1] = i.y, r[n + 2] = i.z;
                    ...
                    this.rendererSystem.applyColorCorrection(s.color),
                ...
                setStars: function (e) {
                    ...
                    0 === e ? this.removeStars() : (this.stars || (this.stars = document.createElement("a-entity"), this.stars.id = "stars", this.createStars(), this.el.appendChild(this.stars)), this.stars.getObject3D("mesh").geometry.setDrawRange(0, e))
                }
            }),
            ...
            var r = function (e) {
                ...
                var r;
                for (r = 0; r < 256; r++)
                    t = parseFloat("0." + Math.sin(9999 * e * r).toString().substr(7)), this.p[r] = Math.floor(256 * t);
                for (this.perm = [], r = 0; r < 512; r++)
